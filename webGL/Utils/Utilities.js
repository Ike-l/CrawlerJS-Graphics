...
export default class Util {
    static Colors = {"RED": [1, 0, 0], "GREEN": [0, 1, 0], "BLUE": [0, 0, 1], "PURPLE": [0.5, 0, 1], "ORANGE": [1, 0.5, 0], "YELLOW": [1, 1, 0], "BLACK": [0, 0, 0], "WHITE": [1, 1, 1]}
    static toRad(degree) {
    ...
    static generateGrid(params = {}) {
        ...
        const color = params.color || Util.Colors.GREEN
        const Grid = []
        for (let i=0;i<=length;i++) {
            Grid.push(new Line({color: color, length:length, center:[offsetX, offsetY, offsetZ+i-length/2]}))
            Grid.push(new Line({color: color, length:length, center:[offsetX+i-length/2, offsetY, offsetZ], rotation:{angle: Util.toRad(90), axis: [0, 1, 0]}}))
        }
        return Grid
    }
    // TODO
