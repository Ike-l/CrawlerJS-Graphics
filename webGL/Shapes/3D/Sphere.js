...
export class Sphere extends Shape {
        constructor(parameters = {}) {
            ...
            super(parameters.center||[0,0,0], [radius, radius, radius], parameters.rotation||{angle: 0, axis: [0, 1, 0]}, gl.TRIANGLES, parameters.material)
            this.radius = radius
        ...
        generate() {
            for (let latNumber = 0; latNumber <= this.quality; ++latNumber) {
                ...
                for (let longNumber = 0; longNumber <= this.quality; ++longNumber) {
                    ...
                    //this.colors.push(Math.abs(x))
                    //this.colors.push(Math.abs(y))
                    //this.colors.push(Math.abs(z))
                    this.colors.push(1, 1, 0)
